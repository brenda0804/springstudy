    ...
    <div>
    	<div class="row">
    		<div class="col-md-12 col-xs-12 col-sm-12">
    			<h2 class="text text-danger">${pspec} 상품</h2>
    		</div>
    	</div>
        <div class="row">
        ...
        <c:if test="${hitList ne null && not empty hitList}">
         <c:forEach var="pd" items="${hitList}" varStatus="st" begin="0" end="3"><!-- 자료구조는 items에 불러와서 pd로 사용 -->
         ...
         <!-- varStatus에 지정된 변수로 반복문의 상태정보를 얻어올 수 있다.
          ...
          <div class="col-md-3 col-xs-6 col-sm-6"><!-- md는 데탑,xs는 스마트폰,sm은 태블릿. -->
           <%--  ${st.index} // ${st.count} --%>
            <a href="pdDetail.do?pnum=${pd.pnum}#shop"><img src="images/${pd.pimage1}" class="img-responsive"></a>
            <h2>${pd.pname}</h2>
            <p>
              <del>
              	<fmt:formatNumber value="${pd.price}" pattern="###,###"/>
              </del> 원<br>
              <span class="text text-danger"><b>
